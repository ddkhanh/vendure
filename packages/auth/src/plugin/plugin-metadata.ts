...
export function getPluginAPIExtensions(
    ...
    const extensions = plugins.map(p => reflectMetadata(p, PLUGIN_METADATA.ADMIN_API_EXTENSIONS));
...
export function graphQLResolversFor(
    ...
    const apiExtensions: APIExtensionDefinition = reflectMetadata(plugin, PLUGIN_METADATA.ADMIN_API_EXTENSIONS);
